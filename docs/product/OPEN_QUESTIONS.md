...
### 2026-08-01 — Q-005 (Atelier Review ownership)

Founder decision: build the Atelier Review feature (UI, checkout, admin review queue) during implementation, but keep it gated off/unsellable until a qualified reviewer is assigned — consistent with `11_COMMERCE_FULFILLMENT_OPERATIONS.md` §1 ("never sell if no qualified reviewer is operationally available"). A feature flag or admin-configured availability toggle should control whether `ATELIER_REVIEW` is purchasable. Reviewer identity remains OPEN and must be resolved before the toggle is switched on; revisit before Phase 4/5 exit and again before Phase 8.
